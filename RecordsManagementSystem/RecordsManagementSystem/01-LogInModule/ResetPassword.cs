...
namespace RecordsManagementSystem
{
    public partial class ResetPassword : UserControl
    {
        ...
        private string empCode, empPW, pwRetype;
        ...
        private void btnResetPassword_Click(object sender, EventArgs e)
        {
            ...
            if (user.EmpPassword == encryptedPW)
            {
                errorProvider.SetError(txtb_password, "Do not use your old password.");
                errorProvider.SetError(txtb_retypePassword, "Do not use your old password.");
            }
            else
            {
                user.EmpPassword = encryptedPW;
                try
                {
                    db.SaveChanges();
                ...
                {
                    MessageBox.Show("Password Reset failed. Try Again.");
                }
            } 
        }
        ...
        {
            TextBox[] textbox = {txtb_employeeCode, txtb_password, txtb_retypePassword };

            foreach (TextBox t in textbox)
            {
                if (string.IsNullOrWhiteSpace(t.Text))
                {
                    enableDisableButton(btnResetPassword, "disable");
            ...
            var user = db.UsersTables.FirstOrDefault(u => u.EmpCode == txtb_employeeCode.Text);
        ...
        private string txtValIsInputEmpty(TextBox txtBox, string data)
        {
            ...
            {
                txtBox.BackColor = Color.FromArgb(13, 65, 71);
                enableDisableButton(btnResetPassword, "enable");
                errorProvider.Clear();
                data = input;
            }
            errorProvider.Clear();
